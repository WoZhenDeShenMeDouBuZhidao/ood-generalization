...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run ACS LAAT-style LLM attribution regularizer baseline.")
    ...
    parser.add_argument("--reg-scale", type=float, default=os.environ.get("REG_SCALE", "1.0"))
    return parser
...
def run_dataset(args: argparse.Namespace) -> ExperimentMetrics:
    ...
    return main(
        ...
        MODEL_NAME="mlp",
        LOSS_NAME="llm_attribution_aligned_ce",
        LOSS_KWARGS=loss_kwargs,
        device=args.device,
        MODEL_SEEDS=MODEL_SEEDS,
        RESULT_PATH=result_output_path("acs", "laat", args.dataset),
        RESULT_METADATA={
            "method": "laat",
...
def main_cli() -> None:
    ...
    for dataset in datasets:
        ...
        print(f"saved {result_output_path('acs', 'laat', dataset)}")
